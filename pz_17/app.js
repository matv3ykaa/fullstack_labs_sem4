...
async function subscribeToPush() {
  ...
  try {
    ...
    const subscription = await registration.pushManager.subscribe({
      ...
      applicationServerKey: urlBase64ToUint8Array('BFKpb4a9jTQOTpjjbceDENHqgNb8TUabD6LewFgdY_ZlAO2KZUQTZi7BrOt2wyXpapkMgpPC_B-5HjM7q-WGVKg')
    });
...
function initNotes() {
  ...
  const input = document.getElementById('note-input-reminder');
...
socket.on('taskAdded', (task) => {
  ...
  function hideToast() {
    ...
    // Удаляем элемент после завершения CSS-перехода
